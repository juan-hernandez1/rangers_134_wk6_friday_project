...
def load_user(user_id):
    ...
    """
    return User.query.get(user_id) # this is a basic query inside our databse to bring back a specific User object
...
# only need this for purposes of tracking what customers are tied to what orders & also how many customer we have
class Customer(db.Model):
    ...
    prodord = db.relationship('ProdOrder', backref = 'customer', lazy=True) # lazy = True means we dont need a ProdOrder table to have a customer
...
class ProdOrder(db.Model):
    ...
    def set_price(self, quantity, price):
        ...
        self.price = quantity * price # this is total price for that product multiplied by quantity purchase
        return self.price
...
products_schema = ProductSchema(many=True) # bringing back all the products in our databse & sending to frontend
